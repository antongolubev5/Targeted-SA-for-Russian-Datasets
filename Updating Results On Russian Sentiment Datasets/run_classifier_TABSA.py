...
def convert_examples_to_features(examples, label_list, max_seq_length, tokenizer):
    ...
    for (ex_index, example) in enumerate(tqdm(examples)):
        ...
        features.append(InputFeatures(
            ...
            input_mask=input_mask,  # единица для норм слов, остальное дополняем нулями до max_seq_length
            segment_ids=segment_ids,  # нули для первого предложения + единицы для второго
            label_id=label_id)  # метка тональности
        )
    return features
...
def main():
    ...
    # train_features = список экземпляров класса InputFeatures (input_ids, input_mask, segment_ids, label_id)
